...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=100, zero_init_residual=False,
        ...
        self.scala1 = nn.Sequential(
            ...
            conv1x1(512, 512 * block.expansion),
        ...
        self.prepare_attention1 = nn.Sequential(
        ...
        self.prepare_attention2 = nn.Sequential(
            conv1x1(128 * block.expansion, 128),
            nn.BatchNorm2d(128),
            nn.ReLU()
        )
        ...
        self.prepare_attention3 = nn.Sequential(
            conv1x1(256 * block.expansion, 256),
            nn.BatchNorm2d(256),
            nn.ReLU()
        )
    ...
    def forward(self, x, level=4):
        ...
        if level==1:
            x1 = self.prepare_attention1(x)
            fea1 = self.attention1(x1)
            fea1 = fea1 * x1
            # feature_list.append(fea1)
            out1_feature = self.scala1(fea1).view(x.size(0), -1)
            return 1, out1_feature, 1, 1, 1

        x = self.layer2(x)

        if level==2:
            x2 = self.prepare_attention2(x)
            fea2 = self.attention2(x2)
            fea2 = fea2 * x2
            # feature_list.append(fea2)
            out2_feature = self.scala2(fea2).view(x.size(0), -1)
            return 2, 2, out2_feature, 2, 2

        x = self.layer3(x)

        if level==3:
            x3 = self.prepare_attention3(x)
            fea3 = self.attention3(x3)
            fea3 = fea3 * x3
            # feature_list.append(fea3)
            out3_feature = self.scala3(fea3).view(x.size(0), -1)
            return 3, 3, 3, out3_feature, 3
        ...
        return out, 4, 4, 4, out4_feature



class ResNet_No_FeatureSlim(nn.Module):

    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None):
        super(ResNet_No_FeatureSlim, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
...
def resnet18(pretrained=False, progress=True, **kwargs):
    ...
    """
    # return ResNet_No_FeatureSlim( BasicBlock, [2, 2, 2, 2])
...
def resnet101(pretrained=False, progress=True, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
        progress (bool): If True, dispflays a progress bar of the download to stderr
    """
...
if __name__ == '__main__':
    ...
    total_ops, total_params = profile(models, (input,), verbose=False)
    print(total_ops / (1000 ** 3), total_params / (1000 ** 2))